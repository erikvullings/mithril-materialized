...
const PaginationItem = () => ({
    view: ({ attrs: { title, href, active, disabled } }) => m('li', { class: active ? 'active' : disabled ? 'disabled' : 'waves-effect' }, typeof title === 'number' ? m('a', { href, oncreate: m.route.link }, title) : title),
});
export const Pagination = () => {
    ...
    return {
        view: ({ attrs: { items, curPage, size = Math.min(9, items.length) } }) => {
            ...
            return m('ul.pagination', displayedItems
                .map((item, i) => ({
                title: startPage + i,
                ...item,
                active: i === curPage,
