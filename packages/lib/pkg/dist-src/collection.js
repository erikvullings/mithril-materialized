    ...
    return {
        view: ({ attrs: { item, mode } }) => {
            ...
            return mode === CollectionMode.AVATAR
                ...
                : m(`li.collection-item${active ? '.active' : ''}`, iconName ? m('div', [title, m(SecondaryContent, item)]) : title);
        },
    };
};
...
export const AnchorItem = () => {
    return {
        view: ({ attrs }) => {
            ...
            return m(`a.collection-item${active ? '.active' : ''}`, { ...params, href, oncreate: href ? m.route.link : undefined }, title);
        },
    };
};
