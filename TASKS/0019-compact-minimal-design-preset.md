...
- 2026-09-10 copilot: Refinement pass tightened actions, fields, choices, ranges, form sections, surfaces, and picker modals; restored circular FAB geometry; fixed selection carets and dark Autocomplete matches; and moved SearchSelect filtering into its trigger line while preserving its combobox semantics and modal focus handling. The Input example no longer double-nests RangeInput grid columns.
- 2026-09-10 copilot: Second refinement normalized icon-only button and ToggleGroup geometry, reduced ConfirmButton, NumberInput, Chips, DatePicker, TimePicker, and Dropdown footprints, aligned TextArea and SearchSelect content, and removed nested choice gutters from Options and RadioButtons. Likert controls now match compact RadioButtons, scaled analog-clock pointer mapping remains accurate, and coarse pointers restore full touch targets.
- 2026-09-10 copilot: Third refinement extends compact density to collections, collapsibles, breadcrumbs, sidenav internals, dialogs, sort indicators, toasts, tabs, pagination, tooltips, and timelines. It also adds fixed menu widths and corrects theme-independent Rating, FileUpload example, LinearProgress example, and SingleRangeSlider alignment.
- 2026-09-10 copilot: Fourth refinement aligns SearchSelect labels and dropdowns with grid content, removes duplicate option text indentation, gives checkbox/radio grid columns proper row gutters, restores Chips-to-divider clearance, centers the scaled analog clock, and matches Likert anchors to compact choice-label typography.
- 2026-09-10 copilot: Fifth refinement raises dialog titles into the close-button band, uses theme-aware primary-action text, resets Breadcrumb's legacy navigation typography, and further reduces DataTable sort indicators.
- 2026-09-10 copilot: Sixth refinement reduces prefixed-field indentation, gives SearchSelect checkbox and plain rows explicit spacing, aligns Fieldset legend text with its description, restores dark-theme badge contrast in the collection example, and separates DataTable sort indicators.
