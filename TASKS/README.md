# TASKS Index

## Foundation and Planning

- [ ] 0001 Library improvement backlog

## Completed Implementation Phases

- [x] 0002 Critical fixes safety and readonly
- [x] 0003 API surface and consistency
- [x] 0004 Reuse primitives extraction
- [x] 0005 CSS layout simplification
- [x] 0006 Async select and combobox primitives
- [x] 0007 Tests and docs alignment

## Open Follow-ups

The following tasks are unblocked and can be worked in parallel.

- [x] 0008 Align form grid defaults
- [x] 0009 ModalPanel style overrides and tokens
- [x] 0010 Compact theme and style override surface
- [x] 0019 Compact minimal design preset *(needs 0010)*

## Overlay and Action Components

Dialog is the shared foundation for CommandPalette. Menu and Snackbar can proceed independently.

- [x] 0011 Dialog and AlertDialog abstraction
- [x] 0012 Menu and context menu
- [x] 0013 Snackbar queue with actions
- [x] 0015 CommandPalette *(needs 0011)*
- [x] 0018 Topmost layer Escape dismissal *(needs 0011, 0012)*

## Content and Identity Components

- [x] 0014 Skeleton and EmptyState
- [x] 0016 Avatar and AvatarGroup

## Large Data

- [x] 0017 Virtualized list and table support